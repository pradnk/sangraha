...
| Sign-in page says no organisation exists yet | Open **`/start`** and create it — whoever does becomes its administrator, and signup shuts behind them. Signup is open only while the database has no organisation, which is what makes this possible with no shell. `npm run db:seed` is for the *demo* organisation in local development and is not what a deployment wants. |
