    ...
    try {
      ...
      throw new Error(
        [
          ...
          '',
          'Create it by hand instead, in the provider\'s SQL editor, with the',
          'password already in DATABASE_APP_URL — then deploy again. This script',
          'grants it everything it needs and will not try to create it again:',
          '',
          `  CREATE ROLE ${username} LOGIN PASSWORD '<the password in DATABASE_APP_URL>';`,
          '',
          'Exactly that, with no attribute clauses. The defaults are all off,',
          'which is what is wanted, and NOBYPASSRLS may only be written by a',
          'superuser — so spelling out the safe thing is itself rejected.',
          '',
          "Do not use Neon's Roles UI for this. A role created there arrives with",
          'attributes that the owner role cannot afterwards remove, and this',
          'script will refuse to migrate against it.',
          '',
          describeError(error),
        ].join('\n'),
      );
  ...
  try {
    ...
    throw new Error(
      [
        ...
        '',
        'Only a superuser can revoke it, so on a managed provider it usually',
        'cannot be fixed in place. This is what a role created through Neon\'s',
        'Roles UI looks like — use its SQL editor instead, where a bare',
        'CREATE ROLE gets the defaults, which are all off.',
        '',
        'Point DATABASE_APP_URL at a fresh role. A new name is easiest, because',
        'dropping this one means clearing its grants first, and the migration',
        'grants whatever name it is given:',
        '',
        "  CREATE ROLE sangraha_app LOGIN PASSWORD '<a password you choose>';",
        '',
        'Then set DATABASE_APP_URL to that role and password, and deploy again.',
        '',
        'If you would rather keep this name, in the SQL editor:',
